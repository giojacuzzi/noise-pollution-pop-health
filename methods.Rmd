...
```{r}
# Configure and check for database
source('global.R')
stopifnot(dir.exists(database_path))

# Retrieve monitoring location information and display
...
source('analysis/noise_regime/evaluate_cumulative_metrics.R') # TODO
```
...
source('analysis/noise_regime/preprocessing/calculate_single_event_metrics.R') # TODO
...
source('analysis/noise_regime/evaluate_single_event_metrics.R') # TODO
```
...
5.  Open 'simulation/DNL/NASWI_Combined_Average_DNL.baseops' with BaseOps. Note that modeled static operations were constant across all four monitoring periods. To import new flight operations into BaseOps:
...
source('analysis/population_health_impacts/preprocessing/calculate_osha_niosh.R') # TODO
```
