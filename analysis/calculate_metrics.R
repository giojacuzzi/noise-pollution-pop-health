...
# Populate `metrics` with analysis metrics for every site ID and date
message('Calculating metrics for each site date')
metrics = data.frame()
num_processed = 0
for (id in unique(data_files$ID)) { # for every site ID
  
  num_processed = num_processed + 1
  name = get_site_name_for_ID(id)
  message(paste0('Processing site ', id, ' \"', name , '\" - ', num_processed, ' of ', length(unique(data_files$ID))))
  
  for (date in unique(data_files[data_files$ID==id, 'Date'])) { # for every date at that site
    
    # Retrieve the measured sound pressure levels
    site_date_data = load_site_date(id, date)
    org = unique(data_files[data_files$ID==id & data_files$Date==date,]$Org)
...
write.csv(metrics, file='data/metrics/metrics_new.csv', row.names=FALSE)