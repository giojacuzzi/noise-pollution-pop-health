...
for (hour in '21') {
  ...
  threshold = 60 # Here, only look at events > 60 dB
  ...
  while (sec<=nrow(data_hour)) {
    ...
    if (data_hour$Lma[sec] > threshold) {
      ...
      event_end = sec
      idx_end = sec
      idx_local_maxima = which(ggpmisc:::find_peaks(data_hour$Lma[idx_start:idx_end]))
      # Only consider prominent maxima > L25 of the event itself
      L25 = LxFromLevels(data_hour[idx_start:idx_end,'LAeq'], 25)
      idx_local_maxima = idx_local_maxima[data_hour[idx_local_maxima, 'Lma'] >= L25]
      ...
      if (length(idx_local_maxima)>1) {
        ...
        for (i in 1:length(idx_local_maxima)) {
          ...
          if (length(idx_local_maxima)>1) {
            ...
            # Wind to last local minimum
            # while (idx_end > idx_start & data_hour$Lma[idx_end]<data_hour$Lma[idx_end-1]) {
            #   idx_end = idx_end - 1
            # }
          ...
          lmax = max(levels)
          idx_lmax = which(data_hour[idx_start:idx_end,'LAeq']==lmax)
          ...
          # # Plot the sub-event with a +/- 10 sec buffer
          # buff_start = max(1, idx_start-10)
          # buff_end = min(nrow(data_hour), idx_end+10)
          # p_time = ggplot(data_hour[buff_start:buff_end,]) +
          #   labs(title=paste(nrow(my_events)),'(split)') +
          #   geom_line(aes(x=Time, y=LAeq)) +
          #   geom_line(aes(x=Time, y=Lma), color='magenta') +
          #   geom_vline(xintercept=data_hour[idx_start,'Time'], color='blue') +
...
for (hour in '21') { # TODO: hours
  ...
  for (q in 1:4) {
    ...
    p_leq = ggplot(data_q) +
      ...
      geom_hline(yintercept=thresh, color='gray') +
