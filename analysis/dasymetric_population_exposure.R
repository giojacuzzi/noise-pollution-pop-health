...
for (c in 1:length(county_dasypops)) {
  message('Reprojecting ', names(county_rasters[[c]]))
  county_rasters_p[[c]] = projectRaster(county_rasters[[c]], template, method = 'ngb')
}
for (c in 1:length(county_dasypops)) {
...
empty_raster = raster(extent(pop_impacted), res = res)
projection(empty_raster) = projection(pop_impacted)
...
island_county_pop_impacted = mask(island_county_pop_impacted, as(contours_Ldn, 'Spatial'))
(cellStats(island_county_pop_impacted, 'sum') / cellStats(county_rasters_p[[1]], 'sum'))

# Population exposure per 5 dB
exposure_levels_Ldn = data.frame()
for (dB in unique(na.omit(as.vector(contours_Ldn_raster)))) {
  message('Exposure ', dB, ' dB')
  exp_masked = mask(pop_impacted, as(contours_Ldn[as.numeric(contours_Ldn$Level)==dB, ], 'Spatial'))
