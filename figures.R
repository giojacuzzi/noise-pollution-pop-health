...
sites_with_events$Org = factor(sites_with_events$Org)

sites_with_events = st_as_sf(sites_with_events,
                 coords = c('Longitude', 'Latitude'), 
                 crs = crs, agr = 'constant')
sites_with_events = na.omit(sites_with_events)
sites_with_events = sites_with_events[sites_with_events$ID %in% unique(get_data_metrics()[,'ID']), ]
sites_with_events$Longitude = st_coordinates(sites_with_events$geometry)[,'X']
sites_with_events$Latitude  = st_coordinates(sites_with_events$geometry)[,'Y']

library(ggrepel)
area_map = ggplot() +
  geom_sf(data = wa_counties_cb) + # aes(fill = value)
  geom_sf(data = contours_polygon[contours_polygon$Level>=45,],
          aes(fill = Level), alpha=0.75) +
  scale_fill_viridis() + # option='magma'
  labs(title = 'Spatial extent of adverse noise exposure', fill = 'Ldn dB(A)', shape = 'Organization') +
  # geom_sf(data = sites, size = 2, shape = 16, color = 'black') +
  geom_sf(data = sites, size = 2, mapping = aes(shape = Org)) +
  # geom_point(data=sites_with_events,
  #            aes(x=x, y= y, color = Org, size = Org)) +
