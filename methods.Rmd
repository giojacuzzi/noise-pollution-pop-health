...
First, set the `database_path` global variable in `global.R` to your local database path. This database should include...
...
# Retrieve site location information
...
# Create tables mapping org files to sites and date
...
# Load SPL data for each site date
...
```{r}
# Calculate metrics for each site date
source('analysis/characterization/preprocessing/calculate_metrics.R')
# Produce 'data/metrics/metrics.csv'
calculate_site_date_metrics_csv()

# Evaluate site index metrics
source('analysis/characterization/site_index_metrics.R')
```

### Single-event acoustic metrics

```{r}
# Find noise events for each site date
...
# Get noise events reported by Navy
# inputs: PHI database
# outputs: 'data/events/_output/navy_reported_events.csv'
source('data/events/load_events_navy.R')

# Evaluate site event metrics
source('analysis/characterization/site_event_metrics.R')
```
...
Conduct OSHA and NIOSH hearing loss impact evaluation. Reminder: uses SPL data

```{r}
# Evaluate OSHA/NIOSH TWA for each site date
source('analysis/health_wellbeing_impacts/preprocessing/calculate_osha_niosh.R')
```
